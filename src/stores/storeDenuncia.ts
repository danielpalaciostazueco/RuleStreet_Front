...
export const useListadoDenuncias = defineStore("listadoDenuncias", () => {
  ...
  async function cargarDatosDenuncias() {
    try {
      ...
      if (!response.ok)
        throw new Error("Error al cargar los datos de las denuncias");
  ...
  async function cargarDatosDenunciasId(denunciaId: number) {
    try {
      ...
      const response = await fetch(
        apiUrl + "/Denuncia/" + denunciaId.toString(),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (!response.ok)
        throw new Error("Error al cargar los datos de la denuncia");
  ...
  async function guardarDenuncia(denuncia: Denuncia) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error al guardar la información de la denuncia: ${errorBody}`
        );
      }
  ...
  async function actualizarDenuncia(denuncia: Denuncia) {
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.text();
        throw new Error(
          `Error al actualizar la información de la denuncia: ${errorBody}`
        );
      }
      ...
      console.error(
        "Error al actualizar la información de la denuncia:",
        error
      );
    }
  }
  ...
  async function borrarDatosDenuncia(denunciaId: number) {
    try {
      ...
      if (!response.ok)
        throw new Error("Error al borrar la información de la denuncia");
      await cargarDatosDenuncias();
    } catch (error) {
  ...
  return {
    cargarDatosDenuncias,
    cargarDatosDenunciasId,
    borrarDatosDenuncia,
    actualizarDenuncia,
    infoDenuncias,
    guardarDenuncia,
