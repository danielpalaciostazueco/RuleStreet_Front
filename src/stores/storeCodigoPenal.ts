...
export const useListadoCodigoPenal = defineStore("listadoCodigoPenal", () => {
  ...
  async function cargarDatosCodigoPenal() {
    try {
      ...
      if (!response.ok)
        throw new Error("Error al cargar los datos del código penal");
  ...
  async function cargarDatosCodigoPenalIdiomaId(codigoPenalId: number) {
    try {
      ...
      const response = await fetch(
        apiUrl + "/CodigoPenal/English/" + codigoPenalId.toString(),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      const index = infoCodigoPenal.findIndex(
        (c) => c.idCodigoPenal === codigoPenalId
      );
      if (index !== -1) {
  ...
  async function cargarDatosCodigoPenalId(codigoPenalId: number) {
    try {
      ...
      const response = await fetch(
        apiUrl + "/CodigoPenal/" + codigoPenalId.toString(),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  async function borrarDatosCodigoPenal(cpId: number) {
    try {
      ...
      if (!response.ok)
        throw new Error("Error al borrar la información del código penal");
  ...
  return {
    cargarDatosCodigoPenal,
    cargarDatosCodigoPenalId,
    borrarDatosCodigoPenal,
    infoCodigoPenal,
    cargarDatosCodigoPenalIdioma,
    cargarDatosCodigoPenalIdiomaId,
  };
});
