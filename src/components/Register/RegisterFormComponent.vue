...
<template>
    <div class="min-h-screen flex justify-center items-center bg-[var(--colorFondo)]"
        style="display: flex; flex-direction: column;">
        <form @submit.prevent="submitForm"
            class="bg-[var(--colorFondoTarjeta)] p-8 rounded-xl shadow-md text-center w-[91.666667%] max-w-[28rem]">
            <h2 class="text-xl md:text-2xl mb-4 text-[var(--colorBlanco)]">{{ $t('Register.Title') }}</h2>
            ...
            <input type="password" v-model="store.DatosRegistro.Contrasena" :placeholder="$t('Register.Password')"
                class="bg-[var(--colorTextoTarjeta)] border border-blue-500 text-black py-2 rounded-md mb-4 w-full text-base">
            <p class="mt-4 text-[var(--colorBlanco)]">
            ...
            <p class="mt-4 text-[var(--colorBlanco)]">
                <RouterLink to="/" class="block mb-2 text-left">{{ $t('Register.Principal') }}</RouterLink>
            </p>
        </form>
